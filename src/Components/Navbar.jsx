...
export function Navbar() {
    const { setCurrent, current } = useContext(mainContext);
    const [inputFocused, setInputFocused] = useState(false);
 
    return (
        <nav className="flex justify-between bg-gray-950 border-b-2 border-white py-1 px-3">
            <div className="flex">
                <button className={`flex items-center cursor-pointer px-2.5 py-2 text-white transition-transform duration-100 hover:text-red-400/70 hover:*:fill-red-400/70 ${current === "videos" && "!text-red-400 *:!fill-red-400 scale-105"}`} onClick={() => setCurrent("videos")}>
            ...
            <div className="flex items-center gap-2">
                ...
                <div className={`flex items-center gap-1 border-b-2 border-white ${inputFocused && "*:fill-red-400 !border-red-400"}`}>
                    ...
                    <input type="text" className="text-[14px] p-1 outline-0 text-white font-semibold" onFocus={() => setInputFocused(true)} onBlur={() => setInputFocused(false)} onChange={"search"} value={"searchValue"}/>
                </div>
            </div>
        </nav>
    );
