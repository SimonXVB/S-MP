...
export function NewPlaylistError({ error, setError }) {
    ...
    return (
        <div ref={errorRef} className="absolute top-full left-0 flex items-center bg-gray-950 text-white w-full mt-2 opacity-0 rounded-md transition-all duration-150">
            ...
            <p className="ml-2 font-bold">
                {error !== "empty" && error !== "exists" && " An error occurred during Collection/Playlist creation"}
                {error === "empty" && " Name field cannot be empty"}
                {error === "exists" && " A Collection/Playlist with this name already exists"}
            </p>
        </div>
    )
};