...
export function NewCollectionInput({ createCollection, setCollectionName, setNewCollectionInput }) {
    ...
    return (
        <form ref={inputRef} onSubmit={e => createCollection(e)} className="absolute left-full top-[50%] -translate-y-[50%] bg-red-400 ml-4 p-5 flex flex-col gap-2.5 rounded-md shadow-xl z-10">
            <fieldset className="border-2 border-white rounded-md">
                <legend className="ml-2 px-1 font-bold text-white">Name</legend>
                <input className="outline-0 px-2 p-1 pt-0 font-bold text-white" type="text" onChange={e => setCollectionName(e.target.value)}/>
            </fieldset>
            <button type="submit" className="flex justify-center items-center rounded-md bg-green-400 cursor-pointer hover:bg-white hover:*:fill-green-400 hover:*:text-green-400">
