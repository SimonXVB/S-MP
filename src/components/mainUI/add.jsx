...
export function Add() {
    ...
    async function copyFile(dir, type) {
        try {
            const res = await window.FS.copyFile(dir, type);
    ...
    return (
        <section className="w-full flex flex-col h-screen">
            ...
            <div className="flex justify-center items-center gap-2 min-h-[400px] h-full overflow-auto text-white px-4">
                <button className="border-4 border-red-400 max-w-[250px] aspect-square w-full flex flex-col justify-center items-center cursor-pointer hover:bg-red-400" onClick={() => copyFile("videos", "videos")}>
                    <img src="../src/assets/addAssets/movie.png"/>
                    <p className="text-2xl font-bold">Add Video</p>
                    <div className="text-[13px]">.mp4, .webm, .ogg</div>
                </button>
                <button className="border-4 border-red-400 max-w-[250px] aspect-square w-full flex flex-col justify-center items-center cursor-pointer hover:bg-red-400" onClick={() => copyFile("audio", "audio")}>
