...
export function RenameModal({name, setModal, rename, onChange, error}) {
    ...
    return (
        <div className="absolute top-0 left-0 z-20 w-full h-full flex justify-center items-center bg-gray-400/40" id="transition">
            <div className="top-[50%] left-[50%] max-w-[400px] w-full rounded-2xl aspect-video bg-gray-700 p-10 text-[20px] text-white font-bold">
                <p className="font-bold">Rename {name}</p>
                ...
                <div className="*:rounded-2xl *:cursor-pointer *:px-4 *:py-2 *:max-w-32 *:w-full flex gap-2">
                    <button onClick={rename} className="bg-green-400 hover:bg-green-400/50">Confirm</button>
